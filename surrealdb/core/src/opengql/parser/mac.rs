...
macro_rules! unexpected {
	($parser:expr_2021, $found:expr_2021, $expected:expr_2021 $(, $($t:tt)* )?) => {{
		let __found: $crate::opengql::token::Token = $found;
		match __found.kind {
...
macro_rules! expected {
	($parser:expr_2021, $($kind:tt)*) => {{
		let token: $crate::opengql::token::Token = $parser.next();
		if let $($kind)* = token.kind {
			token
		} else {
			$crate::opengql::parser::mac::unexpected!($parser, token, $($kind)*)
		}
	}};
}
...
macro_rules! enter_object_recursion {
	($name:ident = $this:expr_2021 => { $($t:tt)* }) => {{
		...
		struct Dropper<'a, 'b>(&'a mut $crate::opengql::parser::Parser<'b>);
		...
		impl<'a> ::std::ops::Deref for Dropper<'_, 'a> {
			type Target = $crate::opengql::parser::Parser<'a>;
